...
class Services extends Component {
                                ...
                                {value.listService.map((values, keys) => {
                                    ...
                                    return (
                                        <div className="item-small relative" key={keys} onClick={() => this.chooseCQQ(values.id, keys)}>
                                            <div className="item-small relative">
                                                {(this.state.keys.some(val => val.key == keys)) == true ? <div className="content-center-middle absolute btn-check h2 bgColorYellow">
                                                    <div className="flex item-center" style={{ width: '85px', justifyContent: 'center' }}>
                                                        <span>Đã chọn</span>
                                                        <span role="img" aria-label="check" className="anticon anticon-check" style={{ marginLeft: '5px' }}>
                                                            <svg viewBox="64 64 896 896" focusable="false" className data-icon="check" width="1em" height="1em" fill="currentColor" aria-hidden="true">
    ...
    total = () => {
        const { services1, list, keys } = this.state;
        var total = list.length + keys.length;
        if (services1 !== -1) {
            total += 1;
        }
        return total;
        // this.setState({totalServices: total})
    }
    ...
    showListId = (services1, keys, list) => {
        let listT = [];
    ...
    render() {
        ...
        const { suggestServices, services, keys, services1, list } = this.state;
        // console.log(this.showListId(services1, keys, list));

        // console.log('totalServices', this.total());
        return (
            <Fragment>
                ...
                listServices={this.showListId(services1, keys, list)} 
