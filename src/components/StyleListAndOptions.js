    ...
    loadHour = () => {
        return this.state.hours.hourGroup && this.state.hours.hourGroup.map((val) =>
            ...
            >
                {
                    val.hours.map(hour =>
                        <div key={hour.hour} style={{ marginRight: '5px' }} className={`box-time_item ${!hour.isFree ? "unavailable" : this.state.hourSelected == hour.hour && "has-choice"}`} onClick={() => this.setHours(hour.hour)}>{hour.hour}</div>
                    )
                }
            </div>
        )
...
export default connect(mapStateToProps, mapDispatchToProps)(StyleListAndOptions);
